...
## Scope
Use this page for EL7062-specific behavior such as warnings, open-loop counter
issues, autotuning, and touch-probe/index homing setup.

## Topics
1. [Error/warning](#errorwarning)
2. [Open-loop counter jump](#open-loop-counter-jump)
3. [Tuning](#tuning)
...
## Open-loop counter jump
The EL7062 has a firmware bug if configured to run in open loop CSV.

When disabling the amplifier, the open-loop counter jumps to the nearest full-turn value.
...
When setting up homing for an encoder in ecmccfg, the latch/touch probe then needs to be configured in the following way:
...
encoder:
  ...
  latch:
    ...
    timestamp: ec0.s$(ENC_SID).touchProbeTimestampPos$(ENC_CH=01)_1 # 32-bit DC time of the same positive edge
    control:  0                                        # Bit in encoder control word to arm latch.
    status: 1                                          # Bit in encoder status word for latch triggered status.
    armCmd: 17                                         # Arm command, 0b10001 for EL7062 touch probe 1 positive edge
...
* encoder.latch.timestamp: optional DC timestamp captured for the same edge

`encoder.latch` is still the correct section when the latch is owned by a
homing sequence. If the same hardware touch-probe should be used as a separate
runtime feature, configure the same style of entries under `encoder.touchProbe`
instead. That keeps normal homing latch behavior separate from the
touch-probe API:
...
encoder:
  touchProbe:
    ...
    timestamp: ec0.s$(ENC_SID).touchProbeTimestampPos$(ENC_CH=01)_1
    control: ec0.s$(ENC_SID).touchProbeControl${ENC_CH=01}
    controlBit: 0
...
For the ED7062 timestamp fields, load `HW_DESC=ED7062_TPDC`. The normal
ED7062/EL7062 descriptors are unchanged. The touch-probe API belongs to the
axis; the encoder mapping above is the hardware adapter. Encoder numbers are
one-based in these commands:

```text
AxisTouchProbeArm(axis,encoder,1)
AxisPrintTouchProbe(axis,encoder)
AxisTouchProbeArm(axis,encoder,0)
```

Without `timestamp`, the result remains usable but is marked cycle-bounded.
With a 32- or 64-bit timestamp it is marked hardware-timestamped. Positive
position and positive timestamp must be paired (or both negative) so they
describe the same physical edge.

### DC/SYNC shift note

The EL7062/ED7062 touch-probe timestamp gives the event time, but the normal
encoder PDO still has to be sampled and transferred at a stable point in the
EtherCAT cycle. The configured DC/SYNC shift can therefore affect the stability
of position reconstruction and position-compare tests.

In one EL7062 test setup, moving the DC shift into the approximately 200 us
range gave more stable results than the previous phase. Treat this as a setup
hint, not a universal default: the useful margin depends on cycle rate, cable
topology, frame length and which terminals are present. After changing the
topology or EtherCAT rate, re-check the timing with `EcPrintSlaveConfig`,
`EcPrintControlTiming` and a touch-probe/position-compare test.
